...
function DoctorProfilePage({ doctorId }) {
...
function ReferenceHome({ activeRoom, setActiveRoom, roomStart, setRoomStart, activeTestimonial, setActiveTestimonial }) {
...
function App() {
	...
	const visibleRooms = rooms.slice(roomStart, roomStart + 4)
	const roomSlideCount = rooms.length - 3
		...
		<div className="site-shell">
			<div className="utility-bar">
				<div className="container utility-inner">
					<span>Rumah Sakit Intan Husada Garut</span>
					<div className="utility-links"><span>Senin - Minggu, 24 Jam</span><a href="tel:02622247769">0262 2247769</a></div>
				</div>
			</div>

			<header className="site-header container">
				<a className="brand" href="#top" aria-label="RSIH home"><img src="/rsih-logo.svg" alt="RSIH Intan Husada" /></a>
				<nav className="main-nav" aria-label="Main navigation">
					{navItems.map((item) => <div className={`nav-item ${item.children ? 'has-dropdown' : ''}`} key={item.label}><a className={(item.page === page || (item.page === 'home' && isHome)) ? 'active' : ''} href={item.href}>{item.label}{item.children && <span className="nav-chevron">⌄</span>}</a>{item.children && <div className="nav-dropdown">{item.children.map((child) => <a href={child.href} key={child.label}>{child.label}</a>)}</div>}</div>)}
				</nav>
			</header>

			<main id="top">
				{!isHome && ['sejarah', 'visi-misi', 'penghargaan'].includes(page) && <AboutPage page={page} />}
				{page === 'rekanan-mitra' && <PartnersPage />}
				{page === 'dokter' && <DoctorsPage />}
				{['jenis-pelayanan', 'rawat-jalan'].includes(page) && <ServicesPage />}
				{page === 'poli-gigi' && <DentalClinicPage />}
				{page === 'fasilitas-umum' && <FacilitiesPage />}
				{page === 'karir' && <CareersPage />}
				{page === 'kontak' && <ContactPage />}
				{page === 'blog' && <BlogPage />}
				{page === 'blog-tonsil' && <BlogDetailPage />}
				{isDoctorProfile && <DoctorProfilePage doctorId={doctorProfileId} />}
				{!isHome && !['sejarah', 'visi-misi', 'penghargaan', 'rekanan-mitra', 'dokter', 'jenis-pelayanan', 'rawat-jalan', 'poli-gigi', 'fasilitas-umum', 'karir', 'kontak', 'blog', 'blog-tonsil'].includes(page) && !isDoctorProfile && <InformationPage page={page} />}
				{isHome && <ReferenceHome activeRoom={activeRoom} setActiveRoom={setActiveRoom} roomStart={roomStart} setRoomStart={setRoomStart} activeTestimonial={activeTestimonial} setActiveTestimonial={setActiveTestimonial} />}
				{isHome && showLegacyHome && <>
				<section className="hero container">
					<div className="hero-copy">
						<p className="eyebrow"><span className="pulse-dot" /> Layanan kesehatan terpercaya di Garut</p>
						<h1>Merawat dengan hati,<br /><em>melayani sepenuh diri.</em></h1>
						<p className="hero-lede">Kami hadir untuk memberikan pengalaman layanan kesehatan yang mudah, profesional, dan manusiawi bagi Anda dan keluarga.</p>
						<div className="hero-actions"><a className="button button-primary" href="#jenis-pelayanan">Temukan layanan <span>↗</span></a><a className="text-link" href="#tentang-kami">Kenali RSIH <span>→</span></a></div>
						<div className="trust-row"><div className="avatar-stack"><span>Dr</span><span>Ns</span><span>Hi</span></div><div><strong>Dipercaya keluarga Garut</strong><small>Profesional sejak 2008</small></div></div>
					</div>
					<div className="hero-visual">
						<div className="hero-image-wrap"><img src="/hospital-hero.svg" alt="Ilustrasi Rumah Sakit Intan Husada" /><div className="image-wash" /></div>
						<div className="hero-note"><span className="note-icon">✚</span><div><strong>Emergency Care</strong><small>Siap melayani 24 jam</small></div><span className="note-arrow">↗</span></div>
						<div className="hero-badge"><strong>15+</strong><span>Tahun melayani<br />masyarakat</span></div>
					</div>
				</section>

				<section className="quick-info container">
					<div><span className="info-icon">◷</span><span><small>Gawat darurat</small><strong>Siaga 24 jam</strong></span></div>
					<div><span className="info-icon">⌖</span><span><small>Lokasi kami</small><strong>Tarogong, Garut</strong></span></div>
					<div><span className="info-icon">♧</span><span><small>Ambulans</small><strong>Siap menjemput</strong></span></div>
					<a className="emergency-link" href="tel:02622800900"><span>Telepon darurat</span><strong>0262 2800900&nbsp; ↗</strong></a>
				</section>

				<section className="intro-section container" id="tentang">
					<div className="section-kicker">01 / Tentang RSIH</div><div className="intro-content"><h2>Lebih dari sekadar<br /><em>tempat berobat.</em></h2><div><p>Rumah Sakit Intan Husada hadir sebagai rumah sakit pilihan masyarakat Garut dan sekitarnya. Kami menggabungkan tenaga medis profesional dengan layanan yang hangat dan penuh perhatian.</p><a className="text-link" href="#kontak">Lebih tentang kami <span>→</span></a></div></div>
				</section>

				<section className="rooms-section" id="kamar"><div className="container"><div className="section-heading"><div><div className="section-kicker">02 / Kenyamanan Anda</div><h2>Pilihan kamar <em>rawat inap.</em></h2></div><div className="room-heading-tools"><p>Ruang yang nyaman dan tenang untuk mendukung proses pemulihan Anda.</p><div className="room-controls"><button onClick={() => setRoomStart(Math.max(0, roomStart - 1))} disabled={roomStart === 0} aria-label="Kamar sebelumnya">←</button><button onClick={() => setRoomStart(Math.min(roomSlideCount - 1, roomStart + 1))} disabled={roomStart === roomSlideCount - 1} aria-label="Kamar berikutnya">→</button></div></div></div><div className="room-grid">{visibleRooms.map((room, index) => { const roomIndex = roomStart + index; return <button className={`room-card ${room.tone} ${activeRoom === roomIndex ? 'selected' : ''}`} key={room.name} onClick={() => setActiveRoom(roomIndex)}><div className="room-image"><img src={room.image} alt={room.name} /><span>0{roomIndex + 1}</span></div><div className="room-body"><h3>{room.name}</h3><p>{room.detail}</p><strong>{room.price}<small> / malam</small></strong><span className="card-arrow">↗</span></div></button> })}</div><div className="room-progress">{Array.from({ length: roomSlideCount }, (_, index) => <button className={roomStart === index ? 'active' : ''} key={index} onClick={() => setRoomStart(index)} aria-label={`Slide kamar ${index + 1}`} />)}</div></div></section>

				<section className="services-section container" id="layanan"><div className="section-heading"><div><div className="section-kicker">03 / Layanan kami</div><h2>Yang Anda butuhkan,<br /><em>kami siapkan.</em></h2></div><p>Pelayanan terpadu dengan standar profesional untuk menjaga kesehatan Anda.</p></div><div className="service-list">{services.map(([icon, title, text], index) => <article className="service-item" key={title}><span className="service-number">0{index + 1}</span><span className="service-icon">{icon}</span><div><h3>{title}</h3><p>{text}</p></div><span className="service-arrow">↗</span></article>)}</div></section>

				<section className="testimonial-section"><div className="container testimonial-layout"><div><div className="section-kicker">04 / Kata mereka</div><h2>Perawatan yang<br /><em>terasa berbeda.</em></h2><div className="rating"><strong>4.9</strong><span>★★★★★</span><small>berdasarkan pengalaman pasien</small></div></div><div className="quote-panel"><span className="quote-mark">“</span><blockquote>{testimonials[activeTestimonial][1]}</blockquote><div className="quote-person"><div className="person-initial">{testimonials[activeTestimonial][0][0]}</div><div><strong>{testimonials[activeTestimonial][0]}</strong><small>Pasien RSIH · {testimonials[activeTestimonial][2]}</small></div><div className="quote-controls"><button onClick={() => setActiveTestimonial((activeTestimonial + testimonials.length - 1) % testimonials.length)} aria-label="Previous testimonial">←</button><button onClick={() => setActiveTestimonial((activeTestimonial + 1) % testimonials.length)} aria-label="Next testimonial">→</button></div></div></div></div></section>

				<section className="contact-section container" id="kontak"><div className="contact-card"><div><div className="section-kicker">05 / Temukan kami</div><h2>Langkah sehat Anda<br /><em>dimulai dari sini.</em></h2><p>Jangan ragu untuk menghubungi tim kami. Kami siap membantu Anda mendapatkan informasi yang dibutuhkan.</p><a className="button button-light" href="https://wa.me/6281214867272">Chat via WhatsApp <span>↗</span></a></div><div className="contact-details"><div><small>Alamat</small><strong>Jl. Mayor Suherman No. 72,<br />Tarogong, Kabupaten Garut</strong></div><div><small>Email</small><strong>rsintanhusada@gmail.com</strong></div><div><small>Sosial media</small><strong>@rs_intanhusada</strong></div></div></div></section>
				</>}
			</main>

			<SiteFooter />
			<a className="floating-contact" href="https://wa.me/6281214867272?text=Halo%2C%20saya%20ingin%20bertanya%20tentang%20layanan%20RSIH" aria-label="Hubungi RSIH melalui WhatsApp"><span className="floating-contact-icon">◔</span><span>Hubungi Kami</span><strong>↗</strong></a>
		</div>
	)
}
